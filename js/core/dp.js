...
(function (global) {
    ...
    DP.config = {
        ...
        morph: {
            ...
            cloudYMin: -0.2,
            cloudYMax: 1.8,
            cloudMix: 0.75,        // 0 — частица держится высоты/радиуса своей пары, 1 — случайная точка кольца
            lift: 0.3,             // подъём частиц ветром

            // --- поле течения (аналог Turbulence Field в Particular) ---
            flowAmp: 0.45,         // сила крупных изгибов вихря
            flowFreq: 0.7,         // масштаб изгибов (больше — мельче)
            flowSpeed: 0.3,        // скорость «дыхания» поля
            jitter: 0.015,         // индивидуальное дрожание частиц
            precession: 0.08,      // покачивание всего вихря
            shiver: 0.012,         // дрожь частиц перед отрывом

            // --- стягивание в перепонки и жгуты (частицы светятся за счёт скучивания) ---
            clumpStrength: 1.0,    // 0 — выкл., 1 — полностью на поверхность
            clumpFreq: 1.1,        // частота структуры: больше — мельче ячейки и тоньше жгуты
            clumpFilaments: 0.8,   // 0 — перепонки (плёнки), 1 — жгуты (линии)
            clumpMaxDist: 0.45,    // насколько далеко частица может «прыгнуть» к поверхности
            clumpFraction: 0.85,   // доля стягиваемых частиц, остальные — пыль вокруг
            clumpSpeed: 0.35,      // скорость, с которой жгуты и перепонки меняют форму (ползут, рвутся, пересоединяются)
            fieldSpin: 0.75,       // поля вращаются вместе с вихрем: 0 — стоят на месте, 1 — со скоростью частиц
            fieldShear: 0.3,       // у оси поле крутится быстрее, чем снаружи (рад/с): наматывает структуры в спирали
